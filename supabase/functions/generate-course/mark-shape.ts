 ...
 */
export function cleanMarks(text: string): string {
  ...
  for (const rule of RULES) {
    for (const position of badMarkerPositions(text, rule, math, doomed)) {
      doomed.add(position);
    }
  }
...
function* badMarkerPositions(
  ...
  for (let index = 0; index + 1 < positions.length; index += 2) {
    ...
    if (wellPlaced(text, open, close, rule)) continue;
    yield open;
    yield close;
  }

  // Un marqueur sans partenaire : il ne veut rien dire et se lirait dans la phrase.
  if (positions.length % 2 === 1) yield positions[positions.length - 1]!;
}
...
/** La marque est-elle posée au bord d'un mot, sur une longueur plausible ? */
function wellPlaced(text: string, open: number, close: number, rule: Rule): boolean {
  const marker = rule.marker;

  const before = open === 0 ? " " : text[open - 1]!;
  if (!BEFORE_OPEN.test(before)) return false;

  const afterIndex = close + marker.length;
  const after = afterIndex >= text.length ? " " : text[afterIndex]!;
  if (!AFTER_CLOSE.test(after)) return false;
  ...
  if (rule.colour && /^\s/.test(inner)) return false;
  ...
  if (inner.includes("\n")) return false;
  if (marker !== "==" && /[.!?]\s/.test(inner)) return false;
